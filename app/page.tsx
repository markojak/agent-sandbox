...
export default function Home() {
  ...
  const oneTapAdd = async (source: "favorite" | "recent", payload: Favorite | Entry) => {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-6 p-6">
      ...
      <section className="rounded border p-4">
        ...
        <form className="grid gap-2 sm:grid-cols-2" onSubmit={submitEntry}>
          ...
          <input
            ...
            value={form.mealTime}
            onChange={(event) => setForm({ ...form, mealTime: event.target.value })}
          />
          <input
          ...
          />
          <button className="rounded bg-black px-4 py-2 text-white sm:col-span-2" type="submit">
            Save entry
          </button>
        </form>
      ...
      <section className="grid gap-4 md:grid-cols-2">
            ...
            {favorites.map((favorite) => (
              <li key={favorite.id} className="rounded border p-2">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <button className="rounded border px-2 py-1" onClick={() => prefill(favorite)}>
                    Prefill
                  </button>
                  <button
        ...
        <div className="rounded border p-4">
          <h2 className="mb-3 text-lg font-semibold">Recents (descending recency)</h2>
          <ul className="space-y-2">
            {recents.map((recent) => (
              <li key={recent.id} className="rounded border p-2">
                <p className="font-medium">{recent.mealName}</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  <button className="rounded border px-2 py-1" onClick={() => prefill(recent)}>
                    Prefill
                  </button>
                  <button className="rounded border px-2 py-1" onClick={() => void oneTapAdd("recent", recent)}>
                    Add now
                  </button>
                </div>
              </li>
          ...
          {entries.map((entry) => {
            ...
            return (
              <li key={entry.id} className="rounded border p-2">
                <p>
                  <span className="font-medium">{entry.mealName}</span> · {entry.calories} cal · {entry.quantity}
                  {entry.isDraft ? " · Draft" : ""}
                </p>
                <div className="mt-2 flex flex-wrap gap-2 text-sm">
                  {favoriteId ? (
                    <button className="rounded border px-2 py-1" onClick={() => void removeFavorite(favoriteId)}>
                      Unfavorite
                    </button>
                  ) : (
                    <button className="rounded border px-2 py-1" onClick={() => void addFavorite(entry.id)}>
                      Favorite
                    </button>
                  )}
                  <button className="rounded border px-2 py-1" onClick={() => void duplicate(entry.id, "draft")}>
                    Duplicate as draft
                  </button>
                  <button className="rounded border px-2 py-1" onClick={() => void duplicate(entry.id, "immediate")}>
                    Duplicate now
                  </button>
                </div>
              </li>
